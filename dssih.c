...
int dssih_free(dssih_t *dssih) {
    ...
    LL_FOREACH_SAFE(dssih->timer_list, timer, timer_tmp) {
        dssih_timer_free(inst);
    }
    free(dssih);
}
...
int dssih_inst_new_by_label(dssih_plugin_t *plugin, const char *label, dssih_inst_t **out_inst) {
    int id;
    for (id = 0; desc = plugin->dssi_desc_fn(id); id++) {
...
int dssih_inst_connect(dssih_inst_t *writer_inst, ulong writer_port_num, dssih_inst_t *reader_inst, ulong reader_port_num, dssih_conn_t **out_conn) {
    ...
    dssih_conn_t conn_stack;
    dssih_conn_t *conn;
    if (writer_port_num >= writer_inst->port_count) {
        DSSIH_RETURN_ERR("dssih_inst_connect: writer_port_num (%d) >= writer_inst->port_count (%d)", writer_port_num, writer_inst->port_count);
    }
    if (reader_port_num >= reader_inst->port_count) {
        DSSIH_RETURN_ERR("dssih_inst_connect: reader_port_num (%d) >= reader_inst->port_count (%d)", reader_port_num, reader_inst->port_count);
    }
    ...
    if (writer->port_desc & LADSPA_PORT_OUTPUT == 0) {
        DSSIH_RETURN_ERR("dssih_inst_connect: writer is not an output");
    }
    if (reader->port_desc & LADSPA_PORT_INPUT == 0) {
        DSSIH_RETURN_ERR("dssih_inst_connect: reader is not an input");
    }
    if ((writer->port_desc & LADSPA_PORT_CONTROL) != (reader->port_desc & LADSPA_PORT_CONTROL)) {
        DSSIH_RETURN_ERR("dssih_inst_connect: type mismatch between writer and reader (control)");
    }
    if ((writer->port_desc & LADSPA_PORT_AUDIO) != (reader->port_desc & LADSPA_PORT_AUDIO)) {
        DSSIH_RETURN_ERR("dssih_inst_connect: type mismatch between writer and reader (audio)");
    }
...
int dssih_inst_disconnect(dssih_conn_t *conn) {
    ...
    writer_inst->dssi_desc->LADSPA_Plugin->connect_port(writer_inst->handle, writer_port_num, &conn->writer->data_null);
    reader_inst->dssi_desc->LADSPA_Plugin->connect_port(reader_inst->handle, reader_port_num, &conn->reader->data_null);
...
int dssih_inst_play(dssih_inst_t *inst, int *notes, int notes_len, int vel, long len_ms) {
    DSSIH_RETURN_ERR("not implemented");
}
...
int dssih_timer_new(dssih_t *dssih, long delay_ms, long interval_ms, long limit, float pct_change, dssih_timer_callback_fn *callback, void *udata, dssih_timer_t **out_timer) {
    ...
    dssih_ms_to_timerspec(delay_ms, &timer->spec.it_value);
    dssih_ms_to_timerspec(interval_ms, &timer->spec.it_interval);
    ...
    timer->udata = udata;
    *out_timer = timer;
    LL_APPEND2(dssih->timer_list, timer, next_all);
    return 0;
}
...
int dssih_timer_set_interval(dssih_timer_t_t *timer, long interval_ms) {
    DSSIH_RETURN_ERR("not implemented");
}

int dssih_timer_set_count(dssih_timer_t_t *timer, int count) {
    DSSIH_RETURN_ERR("not implemented");
}

int dssih_set_midi_handler(dssih_midi_handler_callback_fn_t *callback, void *udata) {
    DSSIH_RETURN_ERR("not implemented");
}
